...
// Determine active session and whether tblreports has sessionId
$activeSessionId = null;
$sesRes = mysqli_query($conn, "SELECT Id FROM tblsessionterm WHERE isActive='1' LIMIT 1");
if ($sesRes && mysqli_num_rows($sesRes) > 0) {
    $rowSes = mysqli_fetch_assoc($sesRes);
    $activeSessionId = (int)$rowSes['Id'];
}
$reportsHasSessionId = false;
$colRes = mysqli_query($conn, "SHOW COLUMNS FROM tblreports LIKE 'sessionId'");
if ($colRes && mysqli_num_rows($colRes) > 0) {
    $reportsHasSessionId = true;
}

// Fetch total reports count with status 'pending' for active session if supported
if ($reportsHasSessionId && $activeSessionId !== null) {
    $queryReports = "SELECT COUNT(*) as totalPendingReports FROM tblreports WHERE status = 'pending' AND sessionId = ".intval($activeSessionId);
    $queryFetchReports = "SELECT * FROM tblreports WHERE status = 'pending' AND sessionId = ".intval($activeSessionId)." ORDER BY created_at DESC LIMIT 5";
} else {
