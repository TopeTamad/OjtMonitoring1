...
// Fetch data from tblreports, prioritizing pending reports over resolved ones; filter by active session if supported
if ($reportsHasSessionId && $activeSessionId !== null) {
    $query = "SELECT id, admissionNumber, fullname, course, report, reply, status, created_at, sent_at 
              FROM tblreports 
              WHERE sessionId = " . intval($activeSessionId) . "
              ORDER BY CASE 
