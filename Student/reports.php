...
// Helper: check if tblreports has sessionId column
...
if (isset($_POST['submit_report'])) {
    ...
    if (empty($course) || empty($report)) {
        ...
        // Prepare and bind, include sessionId if column exists
        if ($reportsHasSessionId && $activeSessionId !== null) {
            $query = "INSERT INTO tblreports (admissionNumber, fullname, course, report, created_at, status, sessionId) VALUES (?, ?, ?, ?, NOW(), 'pending', ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("ssssi", $admissionNumber, $fullname, $course, $report, $activeSessionId);
        } else {
